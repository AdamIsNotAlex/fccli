...
**Implementation/verification notes:** R01 is complete. Readiness arbitrates retained actionable outcomes at the 256-frame drain boundary before yielding to deadlines; focused production-path regressions place 255 lower-priority frames before Close, malformed ack, and matching ack and prove each wins a simultaneously ready subscribe-ack deadline according to the required precedence. Pending REST reconciliation enforces the fixed per-generation span and distinct buffered-key bound before every WebSocket candle insertion or target advance while the REST page remains pending; the bounded deterministic regression uses a test-only limit whose default remains the production 64,000-successor constant. After formatting, the exact gates passed: `cargo test --locked --test hyperliquid_ws_codec --no-default-features --features test-transport` (11/11) and `cargo test --locked --test hyperliquid_live --no-default-features --features test-transport` (20/20).
...
|---|---|---|---|---|
| R01 | [x] | — | no | Complete; formatted exact gates passed: `hyperliquid_ws_codec` 11/11 and `hyperliquid_live` 20/20 |
