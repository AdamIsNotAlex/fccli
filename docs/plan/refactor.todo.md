...
**Implementation/verification notes:** Complete. Reconciliation-target advancement is centralized through the bounded helper for both accepted-watermark branches and candle-driven updates, so the configured per-generation successor limit is checked before mutation even while a REST page is pending. The pending-REST accepted-watermark regression preserves an advance exactly at the configured limit, then advances beyond it and observes the recoverable reconciliation protocol error without completing HTTP. After formatting, the orchestrator passed the exact gates: `cargo test --locked --test hyperliquid_ws_codec --no-default-features --features test-transport` (11/11) and `cargo test --locked --test hyperliquid_live --no-default-features --features test-transport` (21/21).
...
|---|---|---|---|---|
| R01 | [x] | — | no | Complete; formatted exact gates passed: `hyperliquid_ws_codec` 11/11 and `hyperliquid_live` 21/21 |
