...
**Implementation and verification notes:** Complete. Provider-neutral emitter contracts exercise the real shared `EventEmitter` once. The deterministic, `test-transport`-only boundary hook pauses two cloned producers only after both have observed the same control key absent and immediately before the mutex-guarded atomic check-and-reserve boundary; releasing them proves concurrent identical statuses enqueue exactly one envelope, both producers complete, and dequeue permits the key to be reused. A closed-receiver regression repeats the failed enqueue and proves the RAII reservation plus regular/control permits are rolled back after each `ChannelClosed`, so neither stale coalescing state nor permit leakage survives. The hook is cancellation-safe and coverage uses no detached tasks or sleeps.

- [x] Verify: `cargo test --locked --test provider_runtime_websocket --no-default-features --features test-transport` (28 passed).
...
**Implementation notes:** Complete. Shared live supervision remains cut over to `src/provider/runtime/live.rs`; Binance history decoding receives the request and timeframe and validates every decoded row for the subscribed grid, exact fixed/calendar close boundary, available request bounds, and strict increasing/no-duplicate opens before returning any response to shared reconciliation. Production-path Binance live coverage rejects off-grid, inconsistent-close, out-of-window, duplicate, and regressive gap pages before shared reconciliation mutates authoritative state or emits any `ReconcileBatch`. Prior page/span/buffer protections remain implemented. Final formatted gates passed: `binance_rest` 23/23 (supplemental REST decoder coverage), `binance_live` 45/45, `hyperliquid_live` 29/29, and `app_live_contract` 51/51; `provider_contract` 10/10, `history_coordinator` 21/21, and `snapshot_runner` 6/6 remain passed from the same implementation phase.
...
**Implementation notes:** Complete. The shared suite exercises the actual `LiveEngine` for exact reconciliation-ack timeout equality and pre-deadline absence, exact timeout fields, recoverable/backoff ordering, purge, and clean retry; injects stale and insufficient acknowledgements into the runtime receiver before a matching acknowledgement; drops both control senders during reconciliation and connected operation and asserts one terminal in-band `ChannelClosed`, matching producer completion, stream closure, and no retry; covers the complete `1, 2, 4, 8, 16, 30, 30` capped backoff sequence; proves `max(backoff, rate gate)` in both directions with negative early assertions; queues and purges a real generation-scoped candle; and holds the emergency Backoff envelope past the retry deadline to prove the emergency barrier blocks the next connection until both envelopes are dequeued. Narrow test-only acknowledgement injection and generation-invalidation notification expose production state transitions without changing production behavior. Provider suites retain connection-readiness and protocol-policy ownership. `tests/provider_runtime_live.rs` is non-executable mode `100644`. Formatted gates passed: `provider_runtime_live` 15/15, `binance_live` 23/23, `hyperliquid_live` 26/26, and `app_live_contract` 51/51.
...
- [x] Move provider-neutral capped-body, cancellation, timeout mapping, safe-client, rate-gate wait, maximum timed deadline, and absorbing process-block cases from both REST suites into `tests/provider_runtime_http.rs`. Retain provider payload/status/decoder behavior in its provider REST suite. The shared suite exercises the real `HttpRuntime` production path, including request and body I/O connection failures with history recoverability, exact-cap acceptance plus one-byte-over rejection for declared-length and genuinely streamed bodies, direct gate wait/cancellation, common status handling, maximum timed deadline, and absorbing process block; provider REST suites retain Binance `-1121`/418 and Hyperliquid payload/rate semantics without duplicate common contracts.

Implementation note (verified): `runtime/http.rs` uniquely owns safe client construction, cancellation-aware request/body transport mapping, capped body reads, the common response-status skeleton, rate-gate waits, and absorbing/max-deadline updates. `HttpRuntime::read_response` invokes a narrow provider callback only for client-error payload mapping; Binance `-1121`/418 and Hyperliquid payload/rate-limit semantics remain provider-owned. Deterministic production-path loopback coverage verifies request and body I/O connection failures are history-recoverable, and both declared-length and genuinely streamed bodies accept the exact configured cap while rejecting one byte over. Duplicate common provider REST cases remain removed.
...
- [x] Reuse the single public `ProviderCapabilities` type and required `MarketDataProvider::capabilities()` introduced by R05; do not redefine them, add a compatibility method, or introduce another history-limit source. Confirm through `provider_contract` that the public contract remains supported markets, supported timeframes, and a non-zero provider maximum per history request, with no protocol-policy field.
- [x] Route initial app startup, interactive switch preparation, snapshot startup, and `HistoryCoordinator` older-page requests through capabilities before network I/O. Do not modify shared live gap consumption: R05 solely owns its `history_page_limit` behavior and the zero/smaller-limit contracts.
- [x] Preserve `INITIAL_HISTORY_LIMIT = 500` for initial app/switch requests and `SNAPSHOT_HISTORY_LIMIT = 500` for snapshot requests as desired product request sizes; each actual request uses `min(desired_limit, capabilities.history_page_limit)`. Older-history uses the advertised maximum directly. R05 already makes shared live gap use that maximum directly.
- [x] Add market/timeframe preflight to each newly migrated app, switch, snapshot, and older-history consumer, including Binance support and Hyperliquid 1s/6h exclusions, and reject a zero advertised history maximum at those consumers before network I/O. Remove `history::HISTORY_PAGE_LIMIT` and duplicated consumer-side Hyperliquid timeframe-preflight callsites once these remaining consumers use capabilities; R05 has already removed provider-local live `GAP_PAGE_LIMIT` ownership. Retain the two intentional 500-row desired-size constants and provider decoder/request validation as defense in depth.
- [x] Preserve subscription style, heartbeat, finality evidence, rate-limit semantics, connection rotation, and payload retention as internal policies/hooks rather than public trait fields. In particular, expose no `connection_rotation` field in `ProviderCapabilities`; R08 does not change the shared live runtime or provider policy implementations.
- [x] Add contract tests for pre-network market/timeframe/zero-limit rejection at the newly migrated initial app, snapshot, switch, and older-history entry points; complete Binance support; and Hyperliquid exclusions. In `app_live_contract` and `snapshot_runner`, assert the retained desired size is 500 when the provider maximum is at least 500 and is capped to a smaller fake maximum before any request. In `history_coordinator`, assert older-history uses the fake advertised maximum exactly. In `provider_contract`, assert capabilities describe a provider maximum and contain no protocol-policy field. Do not duplicate R05's shared-live zero-limit or exact gap-limit tests in any R08-owned suite.

**Implementation notes:** Complete. Initial app startup, interactive switch preparation, snapshot startup, and older-history requests now validate advertised market/timeframe support and a non-zero provider maximum before network I/O. Initial/switch and snapshot retain desired 500-row requests capped by the advertised maximum; older history uses that maximum exactly. Every new switch command supersedes and cancellation-safely retires pending preparation before provider lookup or capability preflight, so an older accepted target cannot commit after a newer rejected command. Snapshot rejection precedes layout-dependent output, canonicalization, and provider I/O, including undersized TTY output. Deterministic contracts cover the switch 500/min limit matrix, market/timeframe/zero rejection, pending-switch supersession, snapshot pre-output rejection, older-history exact maximum, and real Binance/Hyperliquid capability declarations including Hyperliquid 1s/6h exclusions. `history::HISTORY_PAGE_LIMIT` and duplicated consumer-specific preflight ownership are removed; protocol policies and shared live behavior remain unchanged.
- [x] Verify owned test: `cargo test --locked --test provider_contract --no-default-features --features test-transport` (12 passed).
- [x] Verify owned test: `cargo test --locked --test history_coordinator --no-default-features --features test-transport` (23 passed).
- [x] Verify owned test: `cargo test --locked --test snapshot_runner --no-default-features --features test-transport` (8 passed).
- [x] Verify owned test: `cargo test --locked --test app_live_contract --no-default-features --features test-transport` (54 passed).
- [x] Regression gate only (suite unchanged by R08): `cargo test --locked --test binance_live --no-default-features --features test-transport` (23 passed).
- [x] Regression gate only (suite unchanged by R08): `cargo test --locked --test hyperliquid_live --no-default-features --features test-transport` (26 passed).

### R09 — Generalize ProviderRegistry

**Status:** [x]  
**Depends on:** R08  
**Owned files:** `src/provider/mod.rs`, `src/provider/binance.rs`, `src/provider/hyperliquid.rs`, `src/provider/runtime/http.rs`, `src/main.rs`, `src/app.rs`, `tests/provider_contract.rs`, `tests/app_live_contract.rs`, `tests/binance_live.rs`, `tests/api_boundaries.rs`  
**Parallel safety:** Parallel-safe only against work that touches none of these files; otherwise sequential. The provider implementation ownership is limited to Binance/Hyperliquid xor-cfg purity for the dedicated dual-feature diagnostic, and the shared HTTP ownership is limited to removing the now-unreachable rate-limit arm exposed by that cfg-pure build.  
**Commit boundary:** `refactor(provider): use generic provider registry`

**Implementation note:** Complete. `ProviderRegistry` has one generic `BTreeMap` owner, rejects duplicate IDs on construction and registration, and provides borrowed-ID lookup. Production, app, fake-provider, Binance live, and compile-fixture callers use the ordinary generic path; provider-specific accessors and injection paths are absent. Canonicalization metadata remains independent from registered transports. Review fixes corrected R09 ownership to include the Binance/Hyperliquid transport-only xor-cfg work and the shared HTTP unreachable-arm cleanup required for the dedicated dual-feature diagnostic. Compile fixtures prove `binance`, `with_hyperliquid`, and `with_test_provider` remain unnameable with their specific absent-associated-item diagnostics, while the dual-feature fixture reaches only the intended mutual-exclusion diagnostic.
...
- [x] Remove `with_hyperliquid`, `with_test_provider`, injected-provider special case, and Binance-only accessor in the same commit; prove each removed associated path remains unnameable with specific compile diagnostics.
...
- [x] Test two-provider registration, borrowed lookup, duplicate rejection, unknown/unregistered failure, test-provider registration, and absence of every provider-specific registry path.
...
- [x] Verify: `cargo test --locked --test binance_live --no-default-features --features test-transport` (23 passed).
- [x] Verify: `cargo test --locked --test api_boundaries --no-default-features --features test-transport` (6 passed).

### R10 — Complete protocol/shared-test migration and cleanup

**Status:** [x]  
**Depends on:** R09  
**Owned files:** `src/provider/mod.rs`, `src/provider/test_transport.rs`, `src/provider/binance.rs`, `src/provider/hyperliquid.rs`, `tests/binance_ws_codec.rs`, `tests/hyperliquid_ws_codec.rs`, `tests/binance_rest.rs`, `tests/hyperliquid_rest.rs`, `tests/binance_live.rs`, `tests/hyperliquid_live.rs`, `tests/provider_contract.rs`, `tests/provider_runtime_websocket.rs`, `tests/provider_runtime_live.rs`, `tests/provider_runtime_http.rs`  
...
- [x] Run `cargo test --locked --all-targets --no-default-features --features test-transport` (422 passed across 25 suites; 3 ignored).
...
- [x] Update the accounting table statuses/evidence; all chunks and final gates are complete.
...
**Final gate evidence:** The deterministic emitter boundary-hook and failed-enqueue rollback changes pass the focused `provider_runtime_websocket` gate (28/28). The full test-transport all-targets gate passes 422 tests across 25 suites with 3 ignored; `cargo fmt --check` and test-transport clippy with `-D warnings` pass. Production all-target check and production clippy with `-D warnings` remain passed, as do the feature-selection, mutual-exclusion, API-boundary, and app-state exact gates above.
...
| R04 | [x] | R03 | no | Complete; the shared production `EventEmitter` owns keyed replacement, control saturation/coalescing, emergency ordering/capacity, terminal delivery, and producer completion contracts. A deterministic pre-reservation boundary hook proves two cloned producers that both observed an absent identical key atomically enqueue one envelope and can reuse the key after dequeue; repeated closed-receiver failures prove reservation and both permits roll back. Focused gate passed: `provider_runtime_websocket` 28/28. |
...
| R11 | [x] | R10 | no | Complete; formatting, full test-transport all-targets (422 passed across 25 suites; 3 ignored), test-transport clippy, production all-target check, production clippy, and all exact feature/API/state gates passed. |

**Next dependency-ready unchecked chunk:** None; all tracked chunks are complete.
