...
- For terminal candle slots when plot width is not divisible by candle count, round each cumulative boundary to its nearest ideal fractional position; assigning all remainder columns from one side creates a visible left/right density gradient after horizontal zoom. Keep rendering and pointer interaction on the same geometry.
