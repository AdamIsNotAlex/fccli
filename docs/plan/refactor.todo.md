...
**Implementation and verification notes:** Complete. Provider-neutral transport and emitter contracts execute once in `provider_runtime_websocket`; codec/live suites retain provider protocol and integration behavior. Closure remediation added deterministic provider-live distinct-key saturation integrations to both live suites through real decoded candle frames and each provider's connected loop. A production `saturation_test_hook` synchronizes the saturation transition without sleeps; both scenarios verify generation invalidation, ordered reserved `QueueSaturated`/`Backoff` delivery, retry blocking until the emergency pair is dequeued, and graceful producer/receiver completion. The shared shutdown contract consumes only the first emergency envelope, starts shutdown with the second still queued, synchronizes on `wait_emergency_suppressed`, and proves the stale recovery event is suppressed so `Stopped` is delivered next. After formatting, the exact gates passed: `provider_runtime_websocket` 25/25, `binance_ws_codec` 4/4, `hyperliquid_ws_codec` 11/11, `binance_live` 40/40, and `hyperliquid_live` 27/27.
