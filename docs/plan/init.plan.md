...
| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 13 | `fmt-exec-13` | Retained Main transcript supplies the exact invocation `cargo fmt --check`; `artifact://855` accompanies its PASS with exit 0 and no rustfmt diagnostics on the latest chunk-4 CandleSeries tree. | This append-only execution row supplies current `DET-FMT=PASS`. |
