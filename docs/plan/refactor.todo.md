...
**Implementation notes:** Complete. Initial interactive startup and switch preparation validate the selected market/timeframe and non-zero advertised provider maximum before canonicalization or history I/O, retain the 500-row desired size, and cap each request to the provider maximum. Snapshot performs the same preflight and 500/min behavior before history I/O. `HistoryCoordinator` removed `HISTORY_PAGE_LIMIT`, validates capabilities before spawning an older-history task, and uses the advertised maximum exactly. Owned contract fakes expose configurable capabilities; focused coverage exercises 500 retention, smaller maxima, market/timeframe/zero rejection without requests, switch rejection without a second request/live feed, older-history exact maximum use, and exhaustive public capability-field destructuring so protocol policy cannot be added unnoticed. Shared live runtime and provider policy implementations were not changed. Formatted gates passed: `provider_contract` 11/11, `history_coordinator` 23/23, `snapshot_runner` 8/8, `app_live_contract` 53/53, `binance_live` 23/23, and `hyperliquid_live` 26/26.
- [x] Verify owned test: `cargo test --locked --test provider_contract --no-default-features --features test-transport` (11 passed).
...
- [x] Verify owned test: `cargo test --locked --test app_live_contract --no-default-features --features test-transport` (53 passed).
...
| R08 | [x] | R07 | no | Complete; app startup/switch and snapshot preserve desired 500-row requests capped by the advertised provider maximum, older history uses that maximum exactly, and every migrated consumer rejects unsupported market/timeframe or zero maximum before network I/O. `HISTORY_PAGE_LIMIT` is removed; public capabilities remain data support plus history maximum only, while shared live and provider protocol policy remain unchanged. Formatted gates passed: `provider_contract` 11/11, `history_coordinator` 23/23, `snapshot_runner` 8/8, `app_live_contract` 53/53, `binance_live` 23/23, `hyperliquid_live` 26/26. |
| R09 | [ ] | R08 | no | — |
| R10 | [ ] | R09 | no | — |
| R11 | [ ] | R10 | no | — |
