...
pub trait TerminalInput: Send {
    ...
    ) -> std::io::Result<TerminalInputPoll>;
}
...
`TerminalInput::poll(timeout)` is the sole interactive input boundary. Every call is bounded by the caller-supplied finite timeout and returns exactly `TerminalInputPoll::Event(event)`, `Idle` when the bound elapses without input, `Closed` when the source is permanently exhausted, or a typed I/O failure; it MUST NOT retain a permanently blocking read after the bound. The App owns the sole input worker and its cancellation signal, disables the source after `Closed`, stops further polls on shutdown, and definitively joins that worker before terminal restoration; cancellation latency is therefore bounded by the fixed `INPUT_POLL_TIMEOUT`, and no input task/thread may leak past `TerminalSession` cleanup. Production uses a bounded Crossterm poller, while deterministic tests inject scripted/channel implementations of the same trait. Raw `Box<dyn std::io::Read + Send>`, byte/ANSI decoding in App, detached helper threads, and input ownership of `TerminalDriver` are forbidden.
...
| DET-LINUX-PTY | REQUIRED | PASS | `cargo test --locked --test terminal_lifecycle --no-default-features --features test-transport linux_openpty_restores_kernel_state -- --exact`; exit 0 and named Nix test runs, not zero filtered | never |
...
Chunk-14 append-only formatter update (authoritative over the earlier 100-execution total and `fmt-exec-100` current-basis statement, which remain preserved as historical accounting; these executions belong to chunk 14 snapshot-runner implementation, only retained Main chained commands are counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk14-1` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, first chunk-14 snapshot-runner tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://2496` | exit 0 | PASS; the chain advanced beyond formatting through compilation and multiple test targets to a later `tests/candle_series.rs` failure (`consuming_into_arc_preserves_candle_order_and_content`) | The retained Main chain and `artifact://2496` prove the formatter stage exited 0 because execution advanced to the later test failure. | — | `attempt-2026-08-11-chunk12-16` |
| DET-FMT | `attempt-2026-08-11-chunk14-2` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-14 snapshot-runner tree | `cargo fmt --check` within the retained Main final-success chained command accompanied by `artifact://2509` | exit 0 | PASS; the retained chained command completed successfully after formatting | The retained Main chain and `artifact://2509` prove the formatter stage exited 0 because all later chained stages completed successfully. | — | `attempt-2026-08-11-chunk14-1` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 101 | `fmt-exec-101` | The retained Main command accompanied by `artifact://2496` contains `cargo fmt --check`; the chain advanced through compilation and multiple test targets to a later candle-series test failure, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk14-1` is the execution row. |
| DET-FMT | 102 | `fmt-exec-102` | The retained Main command accompanied by `artifact://2509` contains `cargo fmt --check`; the later chained stages completed successfully, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk14-2` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **102 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-102`, accompanied by Main `artifact://2509`. Main artifacts `artifact://2496` and `artifact://2509` are counted once each in exact order as chunk-14 snapshot-runner executions; subagent runs are excluded. The later test failure and final-success outcomes document chain progression only and do not update unrelated ledger records.

Chunk-14 append-only review note (not a `DET-FMT` execution and not a validation-ledger attempt): a real PTY experiment in a `60×19` tmux pane with `TERM=xterm-256color` wrote successive full-width 60-column rows using the snapshot runner's line-ending pattern. The terminal's autowrap behavior advanced each logical row exactly once; it did not insert blank rows or produce double-spaced output. The review finding that full-width rows necessarily double-space under xterm/tmux autowrap is therefore rejected. This narrow rendering experiment does not execute the production Binance snapshot scenario, does not prove the complete inline-scrollback/prompt/terminal-state contract, and does not update `REAL-SNAPSHOT`, `TERM-LINUX`, or `TERM-TMUX`; all remain `PENDING` until their exact future real-PTY validation semantics are exercised.


Chunk-15 append-only formatter and Linux PTY update (authoritative over the earlier 102-execution total and `fmt-exec-102` current-basis statement, which remain preserved as historical accounting; these formatter executions belong to chunk 15 terminal lifecycle implementation, only retained Main chained commands are counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk15-1` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, first corrected chunk-15 terminal-lifecycle tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://2577` | exit 0 | PASS; the chain advanced beyond formatting, compilation and all tests passed, then the final Clippy stage failed on the unused `Write` import in `src/terminal.rs` | The retained Main chain and `artifact://2577` prove the formatter stage exited 0 because execution advanced through compilation and the full test sequence to the later `cargo clippy --locked --all-targets -- -D warnings` failure. | — | `attempt-2026-08-11-chunk14-2` |
| DET-FMT | `attempt-2026-08-11-chunk15-2` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-15 terminal-lifecycle tree | `cargo fmt --check` within the retained Main final-success chained command accompanied by `artifact://2580` | exit 0 | PASS; the retained chained command completed successfully after formatting, including the final Clippy stage | The retained Main chain and `artifact://2580` prove the formatter stage exited 0 because all later compilation, test, and Clippy stages completed successfully. | — | `attempt-2026-08-11-chunk15-1` |
| DET-LINUX-PTY | `attempt-2026-08-11-chunk15-1` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace with Nix `openpty` support | `cargo test --locked --test terminal_lifecycle --no-default-features --features test-transport linux_openpty_restores_kernel_state -- --exact` | exit 0 and the named Nix test runs, not zero filtered | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 14 filtered out | Main `artifact://2575` records the exact named `linux_openpty_restores_kernel_state` test running and passing. | — | seed |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 103 | `fmt-exec-103` | The retained Main command accompanied by `artifact://2577` contains `cargo fmt --check`; the chain advanced through compilation and all tests to a later Clippy failure on an unused `Write` import, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk15-1` is the execution row. |
| DET-FMT | 104 | `fmt-exec-104` | The retained Main command accompanied by `artifact://2580` contains `cargo fmt --check`; all later compilation, test, and Clippy stages completed successfully, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk15-2` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **104 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-104`, accompanied by Main `artifact://2580`. Main artifacts `artifact://2577` and `artifact://2580` are counted once each in exact order as chunk-15 terminal-lifecycle executions; subagent runs are excluded. The later Clippy failure and final-success outcomes document chain progression only and do not update unrelated ledger records. `DET-LINUX-PTY=PASS` relies on the exact Main command and successful named-test evidence in `artifact://2575`; this automated Nix PTY result does not update any manual real-TTY restoration or platform-terminal record, which remain `PENDING`.

Chunk-15 append-only combined-gate update (authoritative over the earlier 104-execution total, `fmt-exec-104` current basis, and prior `DET-LINUX-PTY` basis, all of which remain preserved as historical accounting; only the retained Main combined command is counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk15-3` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final combined chunk-15 terminal-lifecycle tree | `cargo fmt --check` within the retained Main combined final-success command accompanied by `artifact://2594` | exit 0 | PASS; after the exact Linux PTY test passed, formatting and Cargo check passed, the default-feature suite passed 166 tests, the test-transport suite passed 254 tests, and the final Clippy stage passed | Main `artifact://2594` records the combined command progressing beyond `cargo fmt --check` through every later stage successfully, proving the formatter stage exited 0 without diagnostics. | — | `attempt-2026-08-11-chunk15-2` |
| DET-LINUX-PTY | `attempt-2026-08-11-chunk15-2` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace with Nix `openpty` support, final combined chunk-15 tree | `cargo test --locked --test terminal_lifecycle --no-default-features --features test-transport linux_openpty_restores_kernel_state -- --exact` | exit 0 and the named Nix test runs, not zero filtered | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 17 filtered out | Main `artifact://2594` records the exact named `linux_openpty_restores_kernel_state` test running and passing before the successful formatter, Cargo-check, 166-test default-feature, 254-test test-transport, and Clippy stages. | — | `attempt-2026-08-11-chunk15-1` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 105 | `fmt-exec-105` | The retained Main combined command accompanied by `artifact://2594` contains `cargo fmt --check`; after the exact Linux PTY test passed, the command advanced through successful formatting and Cargo check, 166 passing default-feature tests, 254 passing test-transport tests, and the final Clippy stage, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk15-3` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **105 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-105`, accompanied by Main `artifact://2594`. `DET-LINUX-PTY=PASS` now relies on the superseding exact named-test execution in Main `artifact://2594`; the prior successful Main `artifact://2575` attempt remains preserved as history. The combined Main artifact is counted once, subagent runs are excluded, and its successful Cargo-check, default-feature, test-transport, and Clippy stages do not update unrelated ledger records.

Chunk-15 append-only final combined-gate update (authoritative over the earlier 105-execution total, `fmt-exec-105` current basis, and prior `DET-LINUX-PTY` basis, all of which remain preserved as historical accounting; only the retained Main command is counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk15-4` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, latest final chunk-15 terminal-lifecycle tree | `cargo fmt --check` within the retained Main final-success command accompanied by `artifact://2629` | exit 0 | PASS; the retained command advanced beyond formatting and completed its later compilation, test, and Clippy stages successfully | Main `artifact://2629` accompanies the final successful command and proves the formatter stage exited 0 because all later chained stages completed successfully. | — | `attempt-2026-08-11-chunk15-3` |
| DET-LINUX-PTY | `attempt-2026-08-11-chunk15-3` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace with Nix `openpty` support, latest final chunk-15 tree | `cargo test --locked --test terminal_lifecycle --no-default-features --features test-transport linux_openpty_restores_kernel_state -- --exact` | exit 0 and the named Nix test runs, not zero filtered | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 18 filtered out | Main `artifact://2629` records the exact named `linux_openpty_restores_kernel_state` test running and passing before the later successful stages. | — | `attempt-2026-08-11-chunk15-2` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 106 | `fmt-exec-106` | The retained Main final-success command accompanied by `artifact://2629` contains `cargo fmt --check`; after the exact Linux PTY test passed, the command advanced through successful formatting and all later compilation, test, and Clippy stages, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk15-4` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **106 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-106`, accompanied by Main `artifact://2629`. `DET-LINUX-PTY=PASS` now relies on the superseding exact named-test execution in Main `artifact://2629`, which ran 1 named test with 18 filtered out; the prior successful Main `artifact://2594` and `artifact://2575` attempts remain preserved as history. Main `artifact://2629` is counted once, subagent runs are excluded, and its later successful compilation, test, and Clippy stages do not update unrelated ledger records.

Chunk-16 append-only formatter update (authoritative over the earlier 106-execution total and `fmt-exec-106` current-basis statement, which remain preserved as historical accounting; these executions belong to chunk 16 history-coordinator implementation, only retained Main chained commands are counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk16-1` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, first corrected chunk-16 history-coordinator tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://2688` | exit 0 | PASS; the chain advanced beyond formatting, compilation and tests passed, then the final Clippy stage failed | The retained Main chain and `artifact://2688` prove the formatter stage exited 0 because execution advanced through compilation and tests to the later Clippy failure. | Correct the Clippy finding and rerun the retained Main chain. | `attempt-2026-08-11-chunk15-4` |
| DET-FMT | `attempt-2026-08-11-chunk16-2` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-16 history-coordinator tree | `cargo fmt --check` within the retained Main final-success chained command accompanied by `artifact://2696` | exit 0 | PASS; the retained chained command completed successfully after formatting, including its compilation, test, and final Clippy stages | The retained Main chain and `artifact://2696` prove the formatter stage exited 0 because all later chained stages completed successfully. | — | `attempt-2026-08-11-chunk16-1` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 107 | `fmt-exec-107` | The retained Main command accompanied by `artifact://2688` contains `cargo fmt --check`; the chain advanced through successful compilation and tests to a later Clippy failure, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk16-1` is the execution row. |
| DET-FMT | 108 | `fmt-exec-108` | The retained Main command accompanied by `artifact://2696` contains `cargo fmt --check`; all later compilation, test, and Clippy stages completed successfully, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk16-2` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **108 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-108`, accompanied by Main `artifact://2696`. Main artifacts `artifact://2688` and `artifact://2696` are counted once each in exact order as chunk-16 history-coordinator executions; subagent runs are excluded. The later Clippy failure and final-success outcomes document chain progression only and do not update unrelated ledger records.

Chunk-16 append-only final formatter update (authoritative over the earlier 108-execution total and `fmt-exec-108` current-basis statement, which remain preserved as historical accounting; only the retained Main final-success command is counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk16-3` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final chunk-16 history-coordinator tree | `cargo fmt --check` within the retained Main final-success command accompanied by `artifact://2707` | exit 0 | PASS; the retained Main command completed successfully after formatting | The retained Main command and `artifact://2707` prove the formatter stage exited 0 without diagnostics. | — | `attempt-2026-08-11-chunk16-2` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 109 | `fmt-exec-109` | The retained Main final-success command accompanied by `artifact://2707` contains `cargo fmt --check`; the command completed successfully, so formatting exited 0 without diagnostics. | `attempt-2026-08-11-chunk16-3` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **109 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-109`, accompanied by Main `artifact://2707`. Main `artifact://2707` is counted once as the final chunk-16 history-coordinator execution; subagent runs are excluded, and later successful stages do not update unrelated ledger records.

Chunk-17 append-only formatter and reconciliation-state update (authoritative over the earlier 109-execution total and `fmt-exec-109` current-basis statement, which remain preserved as historical accounting; these formatter executions belong to chunk 17 App/reducer and sole-dispatch implementation, only retained Main chained commands are counted, and subagent runs are excluded):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk17-1` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, first chunk-17 App/reducer tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://2836` | exit 0 | PASS; the chain advanced beyond formatting to the App test target, where 5 tests failed | Main `artifact://2836` proves the formatter stage exited 0 because the retained chain reached the later App-test failures. | Correct the App-test failures and rerun the retained Main chain. | `attempt-2026-08-11-chunk16-3` |
| DET-FMT | `attempt-2026-08-11-chunk17-2` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, corrected chunk-17 App/reducer tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://2865` | exit 0 | PASS; the chain advanced beyond formatting to the App test target, where 1 test failed | Main `artifact://2865` proves the formatter stage exited 0 because the retained chain reached the later App-test failure. | Correct the remaining App-test failure and rerun the retained Main chain. | `attempt-2026-08-11-chunk17-1` |
| DET-FMT | `attempt-2026-08-11-chunk17-3` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final chunk-17 App/reducer and sole-dispatch tree | `cargo fmt --check` within the retained Main final-success command accompanied by `artifact://2963` | exit 0 | PASS; the retained Main command completed successfully after formatting | Main `artifact://2963` proves the formatter stage exited 0 without diagnostics and the retained final chain completed successfully. | — | `attempt-2026-08-11-chunk17-2` |
| DET-RECONCILIATION-STATE | `attempt-2026-08-11-chunk17-1` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final chunk-17 App/reducer tree with test transport | `cargo test --locked --test app_live_contract --no-default-features --features test-transport reconciliation_target_and_state_persistence -- --exact` | exit 0; the exact named aggregate runs and covers the record's stated reconciliation and state-preservation flows | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 19 filtered out | Main `artifact://2965` records the exact authoritative command and named aggregate passing; that aggregate covers every stated `DET-RECONCILIATION-STATE` flow. | — | seed |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 110 | `fmt-exec-110` | The retained Main command accompanied by `artifact://2836` contains `cargo fmt --check`; the chain advanced to the App test target, where 5 tests failed, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-1` is the execution row. |
| DET-FMT | 111 | `fmt-exec-111` | The retained Main command accompanied by `artifact://2865` contains `cargo fmt --check`; the chain advanced to the App test target, where 1 test failed, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-2` is the execution row. |
| DET-FMT | 112 | `fmt-exec-112` | The retained Main final-success command accompanied by `artifact://2963` contains `cargo fmt --check`; the command completed successfully, so formatting exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-3` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **112 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-112`, accompanied by Main `artifact://2963`. Main artifacts `artifact://2836`, `artifact://2865`, and `artifact://2963` are counted once each in exact order as chunk-17 App/reducer and sole-dispatch executions; subagent runs are excluded. The two later App-test failure outcomes document chain progression only and do not update unrelated ledger records. `DET-RECONCILIATION-STATE=PASS` relies only on the exact Main command in `artifact://2965`, where the named aggregate passed 1 test with 19 filtered out and covers the record's stated reconciliation and state-preservation flows. No other ledger record is promoted by these artifacts.

Chunk-17 append-only final formatter and reconciliation-state update (authoritative over the earlier 112-execution total, `fmt-exec-112` current basis, and prior `DET-RECONCILIATION-STATE` basis, all of which remain preserved as historical accounting; only the retained Main commands are credited, subagent runs are excluded, and no other ledger record is promoted):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk17-4` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final chunk-17 App/reducer and sole-dispatch tree | `cargo fmt --check` within the retained Main final-success command accompanied by `artifact://3014` | exit 0 | PASS; formatting and Cargo check passed, the default-feature suite passed 189 tests, the test-transport suite passed 305 tests, the App target passed 28 tests, and the final Clippy stage passed | Main `artifact://3014` records the final successful chain progressing beyond `cargo fmt --check` through every later stage, proving the formatter stage exited 0 without diagnostics. | — | `attempt-2026-08-11-chunk17-3` |
| DET-RECONCILIATION-STATE | `attempt-2026-08-11-chunk17-2` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final chunk-17 App/reducer tree with test transport | `cargo test --locked --test app_live_contract --no-default-features --features test-transport reconciliation_target_and_state_persistence -- --exact` | exit 0; the exact named aggregate runs and covers the record's stated reconciliation and state-preservation flows | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 27 filtered out | Main `artifact://3016` records the exact authoritative command and named aggregate passing on the final chunk-17 tree. | — | `attempt-2026-08-11-chunk17-1` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 113 | `fmt-exec-113` | The retained Main final-success command accompanied by `artifact://3014` contains `cargo fmt --check`; formatting and Cargo check passed, the default-feature suite passed 189 tests, the test-transport suite passed 305 tests, the App target passed 28 tests, and the final Clippy stage passed, so formatting exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-4` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **113 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-113`, accompanied by Main `artifact://3014`. `DET-RECONCILIATION-STATE=PASS` now relies on the superseding exact Main execution in `artifact://3016`, where the named aggregate passed 1 test with 27 filtered out; the prior successful Main `artifact://2965` attempt remains preserved as history. Main artifacts are counted only as stated, subagent runs are excluded, and the successful check, broader test, App-test, and Clippy stages in `artifact://3014` do not promote any unrelated ledger record.

Chunk-17 append-only formatter and reconciliation-state update (authoritative over the earlier 113-execution total, `fmt-exec-113` current basis, and prior `DET-RECONCILIATION-STATE` basis, all of which remain preserved as historical accounting; only retained Main commands are credited, subagent runs are excluded, and no other ledger record is promoted):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk17-5` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, subsequent chunk-17 App/reducer and sole-dispatch tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://3058` | exit 0 | PASS; the chain advanced beyond formatting to the App target, where 34 tests ran with 33 passing and `layout_pending_transitions_on_first_ready_and_later_resize_keeps_chart_data` failing | Main `artifact://3058` proves the formatter stage exited 0 because the retained chain reached the later single App-test failure. | Correct the remaining App-test failure and rerun the retained Main chain. | `attempt-2026-08-11-chunk17-4` |
| DET-FMT | `attempt-2026-08-11-chunk17-6` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-17 App/reducer and sole-dispatch tree | `cargo fmt --check` within the retained Main final-success command accompanied by `artifact://3073` | exit 0 | PASS; formatting and Cargo check passed, the default-feature suite passed 189 tests, the test-transport suite passed 311 tests, the App target passed all 34 tests, and the final Clippy stage passed | Main `artifact://3073` records the final successful chain progressing beyond `cargo fmt --check` through every later stage, proving the formatter stage exited 0 without diagnostics. | — | `attempt-2026-08-11-chunk17-5` |
| DET-RECONCILIATION-STATE | `attempt-2026-08-11-chunk17-3` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-17 App/reducer tree with test transport | `cargo test --locked --test app_live_contract --no-default-features --features test-transport reconciliation_target_and_state_persistence -- --exact` | exit 0; the exact named aggregate runs and covers the record's stated reconciliation and state-preservation flows | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 33 filtered out | Main `artifact://3075` records the exact authoritative command and named aggregate passing on the final corrected chunk-17 tree. | — | `attempt-2026-08-11-chunk17-2` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 114 | `fmt-exec-114` | The retained Main command accompanied by `artifact://3058` contains `cargo fmt --check`; the chain advanced to the App target, where 34 tests ran with 33 passing and 1 failing, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-5` is the execution row. |
| DET-FMT | 115 | `fmt-exec-115` | The retained Main final-success command accompanied by `artifact://3073` contains `cargo fmt --check`; formatting and Cargo check passed, the default-feature suite passed 189 tests, the test-transport suite passed 311 tests, the App target passed all 34 tests, and the final Clippy stage passed, so formatting exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-6` is the execution row and supplies current `DET-FMT=PASS`. |

The canonical total is therefore **115 `DET-FMT` executions**. Current `DET-FMT=PASS` relies on `fmt-exec-115`, accompanied by Main `artifact://3073`. `DET-RECONCILIATION-STATE=PASS` now relies on the superseding exact Main execution in `artifact://3075`, where the named aggregate passed 1 test with 33 filtered out; the prior successful Main `artifact://3016` and `artifact://2965` attempts remain preserved as history. Main artifacts are counted only as stated, subagent runs are excluded, and the successful check, broader test, App-test, and Clippy stages in `artifact://3073` do not promote any unrelated ledger record.

Chunk-17 append-only formatter and reconciliation-state update (authoritative over the earlier 115-execution total, `fmt-exec-115` current basis, and prior `DET-RECONCILIATION-STATE` basis, all of which remain preserved as historical accounting; only retained Main commands are credited, subagent runs are excluded, and no other ledger record is promoted):

| Record ID | Attempt/date | Requirement | Status after attempt | Environment | Command/scenario | PASS semantics | Observed result | Evidence | Blocker/next action | Supersedes |
|---|---|---|---|---|---|---|---|---|---|---|
| DET-FMT | `attempt-2026-08-11-chunk17-7` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, subsequent chunk-17 App/reducer and sole-dispatch tree | `cargo fmt --check` within the retained Main chained command accompanied by `artifact://3111` | exit 0 | PASS; the chain advanced beyond formatting to the App target, where one test failed | Main `artifact://3111` proves the formatter stage exited 0 because the retained chain reached the later single App-test failure. | Correct the remaining App-test failure and rerun the retained Main chain. | `attempt-2026-08-11-chunk17-6` |
| DET-FMT | `attempt-2026-08-11-chunk17-8` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-17 App/reducer and sole-dispatch tree | `cargo fmt --check` within the retained Main final-success command accompanied by `artifact://3134` | exit 0 | PASS; formatting and Cargo check passed, the default-feature suite passed 190 tests, the test-transport suite passed 318 tests, the App target passed all 40 tests, and the final Clippy stage passed | Main `artifact://3134` records the final successful chain progressing beyond `cargo fmt --check` through every later stage, proving the formatter stage exited 0 without diagnostics. | — | `attempt-2026-08-11-chunk17-7` |
| DET-RECONCILIATION-STATE | `attempt-2026-08-11-chunk17-4` / 2026-08-11 | REQUIRED | `PASS` | Linux workspace, final corrected chunk-17 App/reducer tree with test transport | `cargo test --locked --test app_live_contract --no-default-features --features test-transport reconciliation_target_and_state_persistence -- --exact` | exit 0; the exact named aggregate runs and covers the record's stated reconciliation and state-preservation flows | PASS; 1 passed, 0 failed, 0 ignored, 0 measured, 39 filtered out | Main `artifact://3136` records the exact authoritative command and named aggregate passing on the final corrected chunk-17 tree. | — | `attempt-2026-08-11-chunk17-3` |

| Record ID | True occurrence | Canonical execution identity | Exact invocation/result evidence | Historical rows accounted for |
|---|---:|---|---|---|
| DET-FMT | 116 | `fmt-exec-116` | The retained Main command accompanied by `artifact://3111` contains `cargo fmt --check`; the chain advanced to the App target, where one test failed, so formatting necessarily exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-7` is the execution row. |
| DET-FMT | 117 | `fmt-exec-117` | The retained Main final-success command accompanied by `artifact://3134` contains `cargo fmt --check`; formatting and Cargo check passed, the default-feature suite passed 190 tests, the test-transport suite passed 318 tests, the App target passed all 40 tests, and the final Clippy stage passed, so formatting exited 0 without diagnostics. | `attempt-2026-08-11-chunk17-8` is the execution row and supplies current `DET-FMT=PASS`. |
